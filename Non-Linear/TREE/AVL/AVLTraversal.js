class AVLTraversal {
  ...
  async inOrder(node) {
    if (node) {
      ...
      this.traversalResult.push(node.key);
  ...
  async highlightNode(node) {
    ...
    ctx.strokeStyle = 'black'; // Change to black to highlight
    ...
    ctx.fillText(node.key, node.x - 10, node.y + 5); // Display node data

    // Display the current node's data and the traversal array in the output area
    document.getElementById("output-text").innerText = `Current Node: ${node.key} | Traversal Array: [${this.traversalResult.join(", ")}]`;
    ...
    ctx.arc(node.x, node.y, 20, 0, 2 * Math.PI);
    ctx.stroke();
    ctx.fillText(node.key, node.x - 10, node.y + 5); // Draw the data again
  }
  ...
  clearResult() {
    ...
    this.avlTree.drawTree(); // Redraw the tree
  }
  ...
  async printIn() {
    ...
    await this.inOrder(this.avlTree.root);
  ...
  async printPre() {
    this.clearResult(); // Clear previous result
    await this.preOrder(this.avlTree.root);
  ...
  async printPost() {
    this.clearResult(); // Clear previous result
    await this.postOrder(this.avlTree.root);
...
// Create an instance of AVLTraversal
const traversal = new AVLTraversal(avlTree);
