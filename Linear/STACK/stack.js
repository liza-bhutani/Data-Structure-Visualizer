...
class Stack {
  ...
  // Function to display time and space complexity
  displayComplexity(operation) {
    // Time complexity for all stack operations is O(1)
    document.getElementById("time-complexity").innerText = `Time Complexity: O(1)`;

    // Space complexity is proportional to the current size of the stack
    let spaceComplexity = front + 1; // Number of elements in the stack
    document.getElementById("space-complexity").innerText = `Space Complexity: O(${spaceComplexity})`;

    console.log(`${operation} operation performed. Time Complexity: O(1), Space Complexity: O(${spaceComplexity})`);
  }
}
...
document.getElementById("btnSize").addEventListener("click", () => {
  ...
  if (b1 == null) {
    ...
    if (size <= 50 && size >= 5) {
      b1 = new Stack(size);
      popup(true, "Created and empty stack with size " + size + " successfully");
      setSize(size);
    } else {
      popup(false, "Can't make stack for that size. Make stack with less than 50 and greater than 4.");
    }
  } else {
    popup(false, "Stack already exists!");
    if (confirm("Are you sure you want to make a new stack and clear all progress?")) {
      b1 = null;
      front = -1;
      divID = 0;
...
document.getElementById("btnPush").addEventListener("click", () => {
  if (perfoming === true) {
    popup(false, "Let the system complete the running task.");
  } else {
