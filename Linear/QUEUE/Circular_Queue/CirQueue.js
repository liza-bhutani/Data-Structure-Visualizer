...
class Queue {
  constructor(s) {
    ...
    this.displayComplexity(); // Call to display complexity upon queue creation
  }
  ...
  enqueue(data) {
    ...
    if (this.rearIndex == this.size - 1) {
      ...
      this.displayComplexity(); // Display complexity after enqueue operation
      return;
    }
    this.rearIndex++;
  ...
  dequeue() {
    ...
    if (this.frontIndex == this.rearIndex) {
      ...
      this.rearIndex = -1;
      this.queue.pop();
      this.displayComplexity(); // Display complexity after dequeue operation
      return;
    }
  ...
  displayComplexity() {
    // Time complexity for enqueue and dequeue
    let timeComplexity = "O(1)";
    
    // Space complexity
    let spaceComplexity = `O(${this.queue.length})`;
