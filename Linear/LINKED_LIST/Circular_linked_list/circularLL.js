let cl = null;
let arr = [];
let spaceComplexity = 0; // Variable to track the number of nodes
...
class CircularLL {
    ...
    async delete(val) {
        if (this.first === null) {
            popup(false,"can't delete from empty linked list");
            displayCLL();
            return;
        }
        ...
        while (ptr.link !== this.first) {
            if (ptr.data == val) {
                ...
                displayCLL();
                displayTimeComplexity();
                displaySpaceComplexity(); // Update the space complexity display
                return;
            }
            pred = ptr;
            ptr = ptr.link;
